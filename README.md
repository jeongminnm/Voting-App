...
질문 하나와 선택지를 올리면 투표자가 로그인 없이 하나를 골라 투표하고, 결과를 막대그래프로 확인하는 동아리용 웹 투표 앱입니다. 운영자는 별도 로그인으로 전체 투표 현황을 확인할 수 있습니다.
...
- **익명 투표**: 투표자는 로그인 없이 누구나 투표를 만들고 참여합니다. 선택지 하나만 고를 수 있습니다. 중복 투표는 막지 않습니다 ([ADR-0001](docs/adr/0001-anonymous-voting-no-duplicate-prevention.md)).
- **운영자 기능**: 운영자는 `ADMIN_TOKEN`으로 로그인해 읽기 전용 운영자 대시보드에서 전체 투표 현황을 확인하고 로그아웃할 수 있습니다.
...
2. 프로젝트 루트에 `.env.local` 파일을 만들고 Neon 연결 문자열을 넣습니다. 이 파일은 `.gitignore`에 포함되어 저장소에 올라가지 않습니다.
   ...
   ADMIN_TOKEN=<운영자 인증 토큰>
   ```

   운영자 기능을 사용하려면 로컬 `.env.local`에 `ADMIN_TOKEN`을 설정해야 합니다. 배포 환경에서도 같은 이름의 `ADMIN_TOKEN` 환경변수를 설정합니다. 실제 토큰 값은 저장소에 기록하지 않습니다.
